...
<div class="max-w-5xl mx-auto py-8">
    ...
    <div class="flex justify-between items-center mb-6">
        <h2 class="text-2xl font-semibold text-gray-800">Tambah Kontrak & Perjanjian</h2>
    ...
    <form action="{{ route('legal.contracts.store') }}" method="POST" enctype="multipart/form-data" class="bg-white shadow-md rounded p-6 space-y-6">
        @csrf
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="nama_kontrak" value="{{ old('nama_kontrak') }}"
                    class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400" required>
            </div>
...
<select name="jenis_kontrak_id" required class="w-full border border-gray-300 rounded px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400">
    ...
    @foreach($jenisList as $jenis)
        <option value="{{ $jenis->id }}" {{ old('jenis_kontrak_id', $contract->jenis_kontrak_id ?? '') == $jenis->id ? 'selected' : '' }}>
            {{ $jenis->nama_jenis }}
        </option>
    @endforeach
</select>
            ...
            <div>
                ...
                <input type="number" step="0.01" name="nilai_kontrak" value="{{ old('nilai_kontrak') }}"
        ...
        <div class="flex justify-end gap-3">
            ...
            <button type="submit" class="px-6 py-2 bg-blue-600 text-white font-medium rounded hover:bg-blue-700 transition">Simpan</button>
        </div>
